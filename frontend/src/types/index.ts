...
export interface MultiplierRecord {
    ...
    /**
     * Финансовый год отчёта. У компаний со сдвинутым финансовым годом не
     * совпадает с годом даты: у Диасофта период 31.03.2025-31.03.2026 — это
     * отчёт за 2025 год при дате мультипликатора 2026-03-31.
     */
    fiscal_year?: number | null;
    /**
     * Во сколько раз сегодняшний выпуск больше выпуска на дату строки.
     * Отличает дробление от допэмиссии: число акций хранится «как было тогда».
     */
...
export interface CurrentMultipliers {
    ...
    price_used: number | null;
    shares_used: number | null;
    /** Финансовый год отчёта — для подписи строки со сдвинутым фин. годом */
    fiscal_year?: number | null;
    /** Масштаб выпуска к сегодняшнему: отличает дробление от допэмиссии */
    shares_split_factor?: number | null;
    /** Прибыль на акцию, ₽ — от тех же акций, что и капитализация */
    eps: number | null;
    /** Средняя ключевая ставка ЦБ за год отчёта, % */
...
export interface Company {
     ...
     * Мосбиржа хранит котировки под символом, действовавшим в тот день,
     * и связи со старым не даёт — без этой цепочки цена за период до
     * переименования не находится.
     */
    former_tickers?: { ticker: string; until: string }[] | null;
    /**
     * Дробления акций: [{ date: '2026-04-17', ratio: 10 }, …].
     * Цены и количество акций хранятся так, как было тогда; список нужен,
     * чтобы перевести сегодняшний выпуск Мосбиржи в тогдашний.
     */
    share_splits?: { date: string; ratio: number }[] | null;
