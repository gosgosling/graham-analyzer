...
export interface Verdict {
  ...
  distorted: boolean;
  /**
   * Прошла, но по мягкому краю: значение в полосе «приемлемо», а не «хорошо».
   * Без этого одно и то же число красилось зелёным в паспорте и жёлтым в
   * таблице мультипликаторов — у Сбера доля проблемных 7,7% при «хорошо» 4
   * и «плохо» 8.
   */
  marginal: boolean;
  /**
   * Насколько провален порог — в долях самого порога. 0,2 значит «не дотянула
   * пятую часть», 0,78 — «провалила почти целиком». Только у `fail`, только
   * там, где порог числовой. Вердикт от этого не меняется: свод у Грэма
   * двоичный. Но крестик Татнефти с 12% отдачи и крестик Роснефти с 3,2%
   * обязаны выглядеть по-разному — разница между ними четырёхкратная.
   */
