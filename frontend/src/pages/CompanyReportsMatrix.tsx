...
const MATRIX_ROWS: MatrixRowDef[] = [
  ...
  { key: 'provisions', label: 'Расход на резервы ЗА ГОД', kind: 'number', hint: 'млн — строка ОПУ, уменьшает прибыль', only: ['lender', 'hybrid']},
  ...
  { key: 'loan_loss_allowance', label: 'Резерв НАКОПЛЕННЫЙ на дату', kind: 'number', hint: 'млн, положит. — из примечания: валовая − резерв = строка баланса', only: ['lender', 'hybrid']},
  { key: 'npl_loans', label: 'Обесцененные (Стадия 3 + POCI)', kind: 'number', hint: 'млн — ВАЛОВАЯ стоимость Стадии 3, не балансовая', only: ['lender', 'hybrid']},
  ...
  { key: 'cf_other_float', label: 'Δ прочих клиентских обязательств (ОДДС)', kind: 'number', hint: 'млн — второй пул чужих денег: остатки перед продавцами маркетплейса и т.п.', only: ['hybrid', 'exchange']},
