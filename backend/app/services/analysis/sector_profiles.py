...
EXCHANGE = SectorProfile(
    key="exchange",
    label="Биржа / клиринг / депозитарий",
    summary=(
        "Баланс раздут чужими деньгами и зеркальными позициями центрального "
        "контрагента, поэтому D/E и Current Ratio не применяются. Кредитного "
        "портфеля нет — метрики риска банка неприменимы тоже. Оценивается "
        "эффективностью (Cost-to-Income) и свободным потоком без клиентских денег."
    ),
    bands={
        "pe": _band(12, 18, False, "≤ 12 — хорошо для инфраструктуры"),
        "pb": _band(
            3.0, 5.0, False, "≤ 3 — хорошо",
            note=(
                "Балансовый капитал у биржи мал относительно бизнеса: активы — "
                "чужие деньги, а зарабатывает она на комиссиях. P/B здесь "
                "информативнее как ориентир, чем как порог."
            ),
        ),
        "de": _band(
            None, None, False, "D/E для биржи не применим",
            applicable=False,
            note=(
                "Обязательства — средства участников торгов и зеркальные позиции "
                "клиринга: у МОЕХ активы и обязательства ЦК совпадают до рубля. "
                "D/E 40–50× здесь способ учёта, а не плечо."
            ),
        ),
        "cr": _band(
            None, None, True, "CR для биржи не применим",
            applicable=False,
            tooltip_lines=(
                "Оборотные активы и краткосрочные обязательства биржи —",
                "это деньги клиентов и требования клиринга, проходящие",
                "через баланс насквозь. Их отношение не говорит",
                "о платёжеспособности самой биржи.",
            ),
        ),
        "roe": _band(20, 12, True, "≥ 20% — хорошо: капитал у биржи мал, отдача должна быть высокой"),
        "dy": _band(6, 3, True, "≥ 6% — хорошо"),
        "cir": _band(
            45, 55, False, "≤ 45% — эффективная инфраструктура",
            note="Операционные расходы к операционным доходам.",
        ),
    },
)
...
def resolve_sector_profile(
    ...
    `report_type='bank'` и `'exchange'` имеют приоритет: тип задаётся
    аналитиком явно и надёжнее строки сектора из внешнего API.
