...
Поддерживающий капекс в отчётности не выделяется, и разделить его приходится
оценкой — см. `maintenance_split` ниже. Раньше здесь брался капекс целиком, и
это систематически наказывало того, кто строит: у растущей компании половина
трат идёт на новые мощности, а вычиталась она из текущего заработка, как
будто это ремонт.

**Амортизация против поддерживающего капекса.** Амортизация начисляется на
историческую стоимость, а замещать изношенное надо по сегодняшней. При
инфляции разрыв копится, и отчётная прибыль оказывается завышена на его
величину. У Коттла это обязательная проверка, а не тонкость, — поэтому
`depreciation_shortfall` считается для каждого года отдельно и выносится
наверх как признак, а не прячется внутри одной цифры.
...
# Сколько точек нужно, чтобы линия тренда была тенденцией, а не линией через
# шум.
#
# Формально прямая определяется тремя точками, и до сих пор порог стоял там.
# На данных это подвело: у ЛУКОЙЛа за 2023–2025 прибыль на акцию шла 1 668 →
# 1 252 → 158, наклон вышел −755 в год, и тенденция дала **271 ₽** при средней
# за те же годы 1 026 ₽. Один провальный год на трёх точках определяет ответ
# целиком — он и есть половина всей вариации.
#
# У Коттла тренд считается по длинным рядам (гл. 30, табл. 30.3 — десять лет),
# и короткое окно там мерится средней. Пять точек — та граница, ниже которой
# провал не разбавляется ничем.
MIN_TREND_POINTS = 5
...
# ── Разделение капекса, Гринвальд ──────────────────────────────────────────
# Капиталоёмкость — сколько основных средств нужно на рубль выручки. Берётся
# как медиана отношения капекс/выручка по всему ряду: одиночный год для этого
# не годится, там сидит фаза стройки.
#
...
#
# Нижняя граница — ноль: отрицательного поддержания не бывает.

# ── Достаточность амортизации ──────────────────────────────────────────────
# Насколько поддерживающий капекс может превышать амортизацию, чтобы это ещё
# объяснялось инфляцией, а не недоначислением. Замещение по сегодняшним ценам
# при инфляции 7% и среднем сроке службы лет в десять даёт превышение порядка
# трети; всё, что выше, — признак того, что амортизация занижает реальные
# потребности в капитале, и прибыль на эту разницу завышена.
DEPRECIATION_GAP_TOLERANCE = 1.35

# ── Искажение начислениями ─────────────────────────────────────────────────
# Разрыв между операционным потоком и суммой «прибыль + амортизация» — это
# всё, что не является ни заработком, ни износом: высвобождение оборотного
# капитала, неденежные списания, курсовые. Коттл такие статьи из earning
# power исключает.
#
# Во сколько раз разрыв должен превысить свободный поток, чтобы год считался
# искажённым.
#
# **Порог откалиброван по базе, а не назначен.** Сперва здесь стояла треть, по
# рассуждению «разрыв в треть потока — уже много». На 345 годах проверенных
# компаний это дало метку у 66%, то есть величина описывала не исключение, а
# норму: медиана разрыва к потоку составляет 0,59, а верхний квартиль — 1,52.
# Разрыв содержит не только высвобождение оборотного капитала, но и отложенные
# налоги, курсовые, резервы, долю в ассоциированных, — у большинства компаний
# всё это вместе сравнимо со свободным потоком в любой обычный год.
#
# 1,25 выбрано так, чтобы опорный случай остался внутри: у Лукойла за 2025 год
# разрыв 957 млрд против потока 640 — отношение 1,50. Помеченными выходят
# около 28% лет, то есть метка снова означает исключение.
ACCRUAL_GAP_SHARE = 1.25
...
class YearPoint:
    ...
    # ── Разделение капекса и достоверность года, млн ₽ ─────────────────────
    # Хранятся в абсолютных величинах, а не на акцию: это диагностика года, а
    # не ступень оценки, и приводить её к числу акций незачем.
    capex: Optional[float] = None
    capex_maintenance: Optional[float] = None
    capex_growth: Optional[float] = None
    depreciation: Optional[float] = None
    # Во сколько раз поддерживающий капекс превышает амортизацию. Больше
    # единицы — амортизация отстаёт от стоимости замещения.
...
def trend_value(
    ...
    На коротком ряду тенденции не существует — см. `MIN_TREND_POINTS`. Здесь
    возвращается None, и вызывающий берёт среднюю: это не отказ считать, а
    отказ выдавать наклон трёх точек за направление дела.
    """
    by_year = _points_by_year(points)
    known = [y for y, p in by_year.items() if getattr(p, attr) is not None]
    if not known:
...
def load_points(db, company_id: int) -> list:
    ...
    # Разделение капекса считается по всему ряду сразу: капиталоёмкость — это
    # свойство бизнеса, а не года, и брать её из одной строки нельзя.
    upkeep = maintenance_split([
        (report.fiscal_year, _field_rub(report, "revenue"), _field_rub(report, "capex"))
    ...
    for mult, report in rows:
        ...
        gap = None
        distorted = False
        if None not in (operating_cash, profit, depreciation):
            gap = round(operating_cash - profit - depreciation, 3)
            if fcf:
                distorted = abs(gap) > abs(float(fcf)) * ACCRUAL_GAP_SHARE
...
def maintenance_split(rows: list) -> dict:
    ...
    `rows` — список `(год, выручка, капекс)`, все величины в млн ₽.
    Возвращает `год → (поддерживающий, ростовой)`.

    **Приём Гринвальда.** Поддерживающий капекс в отчётности не выделяется, и
    выделить его точно нельзя — но можно оценить сверху. Рассуждение такое:
    если выручка выросла, часть трат ушла на мощности под этот прирост, и
    величина её примерно равна приросту выручки, умноженному на
    капиталоёмкость. Остальное — замещение изношенного.

        капиталоёмкость = медиана(капекс / выручка) по ряду
        ростовой        = капиталоёмкость × (выручка − выручка прошлого года)
        поддерживающий  = капекс − ростовой

    Медиана, а не среднее: в ряду почти всегда есть год большой стройки, и
    среднее он сдвигает, а медиана — нет.

    **Оба ограничения существенны.** Ростовой зажат снизу нулём: при падающей
    выручке формула дала бы отрицательный ростовой капекс и поддерживающий
    выше фактического — то есть компания «поддерживала» больше, чем потратила
    вообще. Сверху ростовой зажат самим капексом: год, где выручка скакнула
    вдвое, иначе объявил бы поддержание нулевым, и прибыль владельца
    сравнялась бы с прибылью плюс амортизация.

    Именно нижнее ограничение даёт правильный ответ на сжимающейся компании.
    У Лукойла за 2025 год выручка упала, ростовой капекс вышел нулём, и весь
    капекс в 775 млрд засчитан как поддерживающий — что и верно: добыча
    снижается, значит ничего не строится, тратится на замещение.

    Без ряда разделить нечем, и тогда возвращается пусто — вызывающий обязан
    откатиться к прежнему поведению и вычитать капекс целиком.
    """
    usable = [
        (year, float(revenue), float(capex))
        for year, revenue, capex in rows
        if revenue is not None and capex is not None
        and float(revenue) > MIN_REVENUE_FOR_INTENSITY
    ]
    if len(usable) < 2:
        return {}

    ratios = sorted(capex / revenue for _, revenue, capex in usable)
    middle = len(ratios) // 2
    intensity = (
        ratios[middle] if len(ratios) % 2
...
def distortion_summary(points: Iterable[YearPoint], window: int,
                       end_year: Optional[int] = None) -> dict:
    """Сколько лет в окне искажены начислениями — и можно ли их выбросить.

    Соблазн — просто выкинуть искажённые годы из нормализации. Он неверен, и
    вот почему. Разрыв «опер. поток минус прибыль с амортизацией» ловит не
    только высвобождение оборотного капитала, но и неденежные списания, а
    списания — часть того, чем компания на самом деле занималась. Выбрасывать
    их значит делать ровно то, против чего «Анализ ценных бумаг» предостерегает
    прямо: аналитик вычитает плохое чаще, чем хорошее.

    Поэтому ряд не режется. Считается доля искажённых лет, и она идёт в
    надбавку за риск: компания, у которой половина окна держится на движении
    оборотки, действительно менее предсказуема, и платить за неё надо меньше.
    Один-два года из семи — обычное дело и надбавки не стоят.
    """
    rows = sorted(points, key=lambda p: p.year)
    if end_year is not None:
        rows = [p for p in rows if p.year <= end_year]
    rows = rows[-window:]
    if not rows:
        return {"years": [], "share": None, "window": window}

    flagged = [p.year for p in rows if p.distorted]
    return {
        "years": flagged,
        "count": len(flagged),
        "of": len(rows),
        "share": round(len(flagged) / len(rows), 3),
        "window": window,
    }
...
def retention_test(points: Iterable[YearPoint], window: int,
                   end_year: Optional[int] = None) -> Optional[dict]:
    """Доходит ли удержанная прибыль до балансового капитала. Тест Баффета.

    Письмо акционерам 1984: «на каждый удержанный доллар компания обязана
    создать не меньше доллара рыночной стоимости». Проверяемая часть этого
    требования — балансовая: прибыль, которую не роздали, обязана осесть в
    капитале. Если не осела, она куда-то делась.

        удержано = Σ (прибыль на акцию − дивиденд на акцию)
        прирост  = балансовая стоимость на акцию, конец − начало
        доля     = прирост ÷ удержано

    Доля около единицы — удержанное осело целиком. Заметно ниже — капитал
    вытекает мимо владельца: списаниями, выводом через связанные стороны,
    переоценками вниз, дивидендами дочек мимо материнской. Различить эти
    причины расчёт не может и не должен — он показывает, что деньги не дошли,
    а почему, выясняет читатель.

    **Обратное тоже бывает и тоже подозрительно.** Доля сильно выше единицы
    означает, что капитал вырос не из прибыли — переоценкой, эмиссией,
    приобретением. Это не заслуга дела, и в рост через удержание такой прирост
    записывать нельзя.

    Тест имеет смысл только при положительном удержании: компания, раздавшая
    больше, чем заработала, капитал и должна была потерять.
    """
    rows = sorted(points, key=lambda p: p.year)
    if end_year is not None:
        rows = [p for p in rows if p.year <= end_year]
    ...
    for point in rows:
        if point.eps is None:
            continue
        dividend = point.dividends_per_share or 0.0
        retained += float(point.eps) - float(dividend)
        counted += 1
    if counted < 3 or retained <= 0:
        return None

    first = next((p for p in rows if p.book_value_per_share is not None), None)
    last = next((p for p in reversed(rows) if p.book_value_per_share is not None), None)
    if first is None or last is None or first.year == last.year:
        return None

    growth = float(last.book_value_per_share) - float(first.book_value_per_share)
    share = growth / retained

    return {
        "retained_per_share": round(retained, 2),
        "book_growth_per_share": round(growth, 2),
        "share": round(share, 3),
