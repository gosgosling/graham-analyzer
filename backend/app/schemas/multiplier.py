...
class MultiplierResponse(BaseModel):
    ...
    # Финансовый год отчёта. Не совпадает с годом даты у компаний со сдвинутым
    # финансовым годом: у Диасофта период 31.03.2025-31.03.2026 — это отчёт за
    # 2025 год, а дата мультипликатора 2026-03-31. Подписывать строку по дате
    # значит сдвинуть всю историю на год вперёд.
    fiscal_year: Optional[int] = None
    # Во сколько раз сегодняшний выпуск больше выпуска на дату строки.
    # Число акций хранится «как было тогда», поэтому без этого коэффициента
    # дробление 10:1 у Т-Технологий читается как размытие доли в десять раз,
    # а допэмиссия и дробление выглядят одинаково.
