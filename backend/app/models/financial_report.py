...
class FinancialReport(Base):
    ...
    # Прочие нематериальные активы из баланса, млн — БЕЗ гудвила, он отдельным
    # полем. Купленные лицензии, софт, патенты, клиентская база, отложенные
    # права. Из материального капитала (pb_tangible) НЕ вычитаются: лицензии и
    # софт — настоящие средства производства. Нужны, чтобы видеть, какая доля
    # капитала держится на нематериальном, — среди этих строк попадаются и
    # условные, живущие до тех пор, пока выполняются условия сделки.
    intangible_assets: Mapped[Optional[float]] = mapped_column(Numeric(15, 3), nullable=True)
    ...
    # Δ прочих клиентских обязательств из ОДДС, млн — второй пул чужих денег.
    #
    # У маркетплейса деньги покупателя лежат на счетах, пока продавцу их не
    # перечислили: у Озона это 339 786 млн на балансе против 505 788 млн
    # депозитов Финтеха. Прирост такого флоата надувает операционный поток
    # ровно как прирост депозитов, и вернуть его придётся так же.
    #
    # Пусто у компаний с одним пулом (Яндекс, Мосбиржа) — тогда приток
    # считается по депозитам и кредитам, как раньше.
    cf_other_float: Mapped[Optional[float]] = mapped_column(
        Numeric(15, 3), nullable=True
    )
